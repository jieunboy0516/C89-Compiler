...
public:
	ConstantValue(int value_in) : value(value_in){};
	...
	std::string codeprint(Context &cont) override
	{
		...
		if (value != 0)
		{
			ss << "li t0, 0 #store true to t0 \n"; // store tuthieness to t0
		}
		else
		{
			ss << "li t0, 1 #store false to t0 \n";
		}
...
public:
	Identifier(std::string name_in) : name(name_in){};
	std::string print();
	std::string cprint();
...
public:
	BinaryExpression(Expression *left_in, const Operator op_in, Expression *right_in) : left(left_in), op(op_in), right(right_in){};
...
private:
	std::string name;
	Expression *assignmentexpr;
	int index; // index of the array to be changed
	bool isRedeclaration;
	Operator op;

public:
	ArrayAssignmentExpression(std::string name, Expression *assignmentexpr, Operator op_in, int index, bool isRedeclaration) : name(name), assignmentexpr(assignmentexpr), op(op_in), index(index), isRedeclaration(isRedeclaration)
																															   {};
